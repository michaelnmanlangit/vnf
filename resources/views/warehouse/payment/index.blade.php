...
<div class="inventory-grid-container">
    ...
    <div class="inventory-toolbar">
        <form method="GET" action="{{ route('warehouse.payment.index') }}" id="filterForm" class="toolbar-form">
            <!-- Search Box -->
            ...
            <div class="toolbar-actions">
                @if(request('search'))
                    <a href="{{ route('warehouse.payment.index') }}" class="clear-filters">Clear All</a>
                @endif
            </div>
        </form>
    </div>
    ...
    @if($inventory->count() > 0)
        <div class="inventory-list">
            <table class="inventory-table" data-route="{{ route('warehouse.payment.index') }}">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Product Name</th>
                ...
                <tbody>
                    ...
                    <tr class="inventory-row">
                        ...
                        <td>₱{{ number_format(($item->unit_cost ?? 0) * $item->quantity, 2) }}</td>
                        <td>₱{{ number_format($item->selling_price ?? 0, 2) }}</td>
                        <td>{{ $item->updated_at->format('M d, Y') }}</td>
                        <td class="actions-cell">
                            <a href="{{ route('warehouse.payment.show', $item) }}" class="btn-action view" title="View Details">
    ...
    @else
        ...
        <div class="empty-state">
            ...
            <h2>No Items</h2>
            <p>No inventory items available for payment management.</p>
        </div>
    @endif
